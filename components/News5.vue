<template>
  ...
  <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1"/>
      ...
      <!-- Правая колонка для последних новостей -->
      <div class="col-12 col-md-3 fixed-sidebar mb-4 mb-md-0">
        <h3 v-if="!isLoading" class="fw-bold">Выбор редакции</h3>
        <div class="sidebar-content">
          <div v-for="(newsItem, index) in latestNews.slice(25)" :key="index" class="sidebar-news-item">
            <p><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-circle" viewBox="0 0 16 16">
  <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0"/>
  <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1"/>
</svg> {{ newsItem.author }}</p>
...
export default {
  data() {
    return {
      ...
      latestNews: [], 
      currentPage: 1,
      pageSize: 8, // Увеличили количество новостей на страницу до 8
  ...
  async mounted() {
    ...
    this.isLoading = false; // Устанавливаем в false после загрузки данных
  },
  methods: {
    async fetchNews() {
      try {
    ...
    async fetchLatestNews() {
      try {
        const response = await fetch(
          "https://4v-news-api.azurewebsites.net/News?SiteId=1&LanguageCode=ru&CategoryId=19&Page=1&PageSize=40"
        );
        const data = await response.json();
